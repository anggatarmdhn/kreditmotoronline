...
<head>
    ...
    <title>Pembayaran DP – {{ $companyName }}</title>
...
<body class="flex items-center justify-center p-4">
    ...
    <div class="card max-w-md w-full p-8">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-extrabold text-slate-900">Pembayaran Uang Muka</h1>
            <p class="text-slate-500 text-sm mt-1">Selesaikan pembayaran DP untuk kredit motor Anda</p>
        </div>

        <!-- Info Pengajuan -->
        <div class="bg-slate-50 rounded-2xl p-5 mb-6 border border-slate-100">
            <div class="info-row">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wide">Kode Pengajuan</span>
                <span class="font-mono font-bold text-slate-800 text-sm">{{ $pengajuan->kode_pengajuan }}</span>
            </div>
            <div class="info-row">
            ...
            <div class="info-row">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wide">Tenor</span>
                <span class="font-bold text-slate-800 text-sm">{{ $pengajuan->tenor_bulan }} Bulan</span>
            </div>
            <div class="info-row">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wide">Angsuran/Bln</span>
                <span class="font-bold text-slate-800 text-sm">Rp {{ number_format($pengajuan->angsuran_per_bulan, 0, ',', '.') }}</span>
            </div>
            <div class="mt-4 pt-4 border-t border-slate-200">
                <div class="flex justify-between items-center">
                    <span class="font-bold text-slate-600">Total DP yang harus dibayar</span>
                    <span class="text-2xl font-black text-emerald-600">Rp {{ number_format($pengajuan->dp, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <a href="{{ route('client.pengajuan') }}" class="block text-center text-sm text-slate-400 hover:text-slate-600 font-semibold mt-4 transition">
            ← Kembali ke Pengajuan Saya
        </a>
    </div>

<script>
const KONFIRMASI_URL = "{{ route('client.bayar.konfirmasi', $pengajuan) }}";
const SELESAI_URL    = "{{ route('client.bayar.selesai', $pengajuan) }}";
const PENGAJUAN_URL  = "{{ route('client.pengajuan') }}";
...
/**
 * Kirim konfirmasi pembayaran ke server via AJAX POST.
 * Ini dipanggil setelah Snap melaporkan pembayaran berhasil/pending.
 * Server akan redirect ke halaman sukses setelah konfirmasi berhasil.
 */
...
payBtn.addEventListener('click', function () {
    ...
    window.snap.pay(SNAP_TOKEN, {
        onSuccess: function (result) {
            // Snap melaporkan sukses → konfirmasi ke server via AJAX
            kirimKonfirmasi(result.transaction_status || 'settlement', result.order_id || '');
        },
        onPending: function (result) {
            // Pembayaran pending (transfer bank belum settlement)
        ...
        onClose: function () {
            // User menutup popup Snap — tidak ada aksi, cukup reset tombol
            setLoading(false);
        },
    });
});
